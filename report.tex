...
\subsection{Algorithm explanation}\todo[color=green!40]{did't like subsection title-changed}
...
\todo[color=green!40]{changed a little}
...
Cv_i = \lambda_i v_i
\end{equation}
$i=1,...,M$. Our goal is to solve this equation without working directly in the feature space.\todo[color=green!40]{need to explain why we have such goal} Substituting $C$ from (\ref{cov_matrix}) we get
\begin{equation}
...
The key thing here is to express last equation\todo[color = green!40]{got rid of reference, redundant here} in terms of kernel function defined as $k(x_n, x_m) = \phi(x_n)^T\phi(x_m)$. It is done by multiplying both sides by $\phi(x_l)^T$ which results to the next:
...
By solving the problem we find eigenvectors $\bm{\alpha_i}$. \todo[color = green!40]{this sentence can be removed as doesn't ass much value to concept}Note that solutions of (\ref{K_1}) and (\ref{K_2}) differ only by eigenvectors that correspond to zero eigenvalues of K, hence removing $K$ from both sides of (\ref{K_1}) does not affect principal components.  

\todo[color = green!40]{the part needs further discussion}
